...
-- =============================================================================
-- Avaliação anônima dos resultados
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS avaliacoes_analise (
    ...
    data_avaliacao TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_avaliacoes_analise_usuario UNIQUE (analise_id, avaliador_id),
    CONSTRAINT ck_avaliacoes_analise_nota CHECK (nota BETWEEN 1 AND 5)
);
...
SELECT
    ...
    COUNT(*) FILTER (WHERE categoria = 'neutra')::INTEGER AS neutras,
    COUNT(*) FILTER (WHERE categoria = 'negativa')::INTEGER AS negativas,
    COALESCE(ROUND(100.0 * COUNT(*) FILTER (WHERE categoria = 'positiva') / NULLIF(COUNT(*), 0), 2), 0) AS percentual_positivas,
    COALESCE(ROUND(100.0 * COUNT(*) FILTER (WHERE categoria = 'neutra') / NULLIF(COUNT(*), 0), 2), 0) AS percentual_neutras,
    COALESCE(ROUND(100.0 * COUNT(*) FILTER (WHERE categoria = 'negativa') / NULLIF(COUNT(*), 0), 2), 0) AS percentual_negativas,
